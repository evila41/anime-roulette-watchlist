...
<template>
  <main
    class="min-h-screen bg-[radial-gradient(circle_at_15%_20%,#4c1d95,transparent_40%),radial-gradient(circle_at_85%_0%,#312e81,transparent_28%),linear-gradient(160deg,#020617,#1e1b4b,#312e81)] px-4 py-8 text-indigo-100 sm:px-6 lg:px-8"
  >
    <div class="mx-auto max-w-7xl">
      <header class="mb-8">
        <p class="text-xs font-semibold tracking-[0.3em] text-purple-300 uppercase">Project #4</p>
        <h1 class="mt-2 text-4xl font-black text-white sm:text-5xl">Anime Roulette Machine</h1>
        <p class="mt-2 max-w-3xl text-sm text-indigo-200 sm:text-base">
          ...
          >
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                ...
                class="cursor-pointer rounded-full border border-purple-300/70 bg-purple-500/20 px-6 py-3 text-base font-black tracking-wide text-purple-100 hover:bg-purple-500/30 disabled:cursor-not-allowed disabled:opacity-60"
                @click="spin"
              >
                {{ spinLabel }}
              </button>
            ...
            <p
              v-if="cooldownLeft > 0"
              class="mt-4 rounded-xl border border-yellow-300/50 bg-yellow-400/10 px-3 py-2 text-sm font-semibold text-yellow-100"
            >
